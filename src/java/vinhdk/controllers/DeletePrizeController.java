...
public class DeletePrizeController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            bean.setPrizeId(id);
            boolean check = bean.delete();
            if (check) {
                request.setAttribute("VALID", "Delete Success");
            } else {
                request.setAttribute("VALID", "Delete Fail");
            }
        } catch (Exception e) {
            if (e.getMessage().contains("REFERENCE")) {
                request.setAttribute("VALID", "Can't delete");
            } else {
                log("Error at DeleteController " + e.getMessage());
            }
        } finally {
            request.getRequestDispatcher("AwardController").forward(request, response);
        }
    }
